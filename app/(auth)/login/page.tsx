...
// Separado em componente próprio pois useSearchParams() requer Suspense no App Router
function LoginForm() {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      const defaultRoute = data.role === 'PLATFORM_ADMIN' ? '/painel-admin' : '/dashboard'
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-950">
      <div className="w-full max-w-sm">
        ...
        <Suspense fallback={<div className="bg-slate-900 border border-slate-800 rounded-xl p-8 h-64 animate-pulse" />}>
          <LoginForm />
        </Suspense>
      </div>
    </div>
